...
def add_outcome_bucket(df,
                       ...
                       offer_col='Offer Selected from Provider or Issuer'):
    """
    ...
    Primary signal: 'Offer Selected from Provider or Issuer' (line-level).
    Fallback for rows where Offer Selected is null: 'Payment Determination
    Outcome' (dispute-level majority). This covers default rows (one party
    didn't submit, so nothing was "selected") and a small number of batched
    dispute lines where CMS did not populate the field.

    Requires 'Default Decision' column. Falls back gracefully if offer_col
    is absent (e.g., older DataFrames that lack the field).
    """
    import numpy as np

    is_default = df[default_col] == 'Yes'

    # ── Primary: Offer Selected (line-level) ─────────────────────────────────
    if offer_col in df.columns:
        offer     = df[offer_col].fillna('')
        prov_sel  = offer.str.contains('Provider', na=False)
        plan_sel  = offer.str.contains('Plan|Issuer', na=False)
        has_sel   = prov_sel | plan_sel
    else:
        # Field not present — treat every row as needing the PDO fallback
        prov_sel = plan_sel = has_sel = pd.Series(False, index=df.index)

    # ── Fallback: Payment Determination Outcome (dispute-level) ──────────────
    pdo      = df[outcome_col].fillna('')
    prov_pdo = pdo.str.contains('Provider', na=False)
    plan_pdo = pdo.str.contains('Plan|Issuer', na=False)

    # Use Offer Selected where available; PDO everywhere else
    is_prov = np.where(has_sel, prov_sel, prov_pdo)
    is_plan = np.where(has_sel, plan_sel, plan_pdo)
